...
def main():
    ...
    while True:
        ...
        for title, lucene, rule in mgr.snapshot(): #sigma rules 리스트를 각 변수에 저장
            ...
            print(f"[LUCENE] {title}: {lucene_fixed}")
            
            #ocsf-log index에 lucene 쿼리 전송 후 결과 값 저장
            try:
                res = es.search(index="ocsf-*", body={
                    "size": 10,
                    "sort": [{"@timestamp": "asc"}],
            ...
            for hit in res["hits"]["hits"]: #검색된 로그 배열을 하나씩 불러와 저장
                ...
                for pattern in patterns: #패턴과 일치하는 이벤트 탐지
                    print(f"[PATTERN MATCH] IP={ip}, PATTERN={pattern['name']}, SEQ={pattern['sequence']}")
                    print("→ 현재 IP 상태:", list(ip_tracker.states.get(ip, [])))
